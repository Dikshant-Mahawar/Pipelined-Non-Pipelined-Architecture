def fetch_instruction(instruction_memory,pc): #Function to fetch instructions
    return instruction_memory[pc]


def binary_neg_decimal(decimal_value):#Function to convert binary to decimal(handles negatives cases also)
    if decimal_value & 0x8000: 
        decimal_value = -((1 << 16) - decimal_value)
    return decimal_value


def decode_instruction(instruction,reg_memory):#Function to decode instructions
    op=instruction[0:6]
    opcodes = { #opcodes dictionary
    "000000":"r_type",
    "100011":"lw" ,
    "101011":"sw",
    "001000":"addi",
    ...
    for e in opcodes:
        if(e==op):
            ...
            if(str=="r_type"):
                ...
                rt1=reg_memory[rt]
                rd1=reg_memory[rd]
                l=[rs1,rt1,rd1,shamt,funct,rs,rt,rd] #list containg required data fetched from instruction
                return l
...
def execute_instruction(l,pc):#Function to execute instructions
...
def memory_access(wm,rm,data_memory,x,rt1):#Function for Data Memory access
    if(rm==1):
...
def write_back(reg_memory,rd1,rd):#Function for write back
    if(rd=='$s0'):
        rd1=abs(rd1)
    reg_memory[rd]=rd1
    return reg_memory
...
#binary for fibonacci
    '''instruction_memory=['00100000000011110000000000000000',
